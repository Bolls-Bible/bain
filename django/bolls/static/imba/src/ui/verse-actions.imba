...
tag verse-actions < section
	...
	get routeBibleShareUrl
		...
		return "https://route.bible/{bookCode}.{activities.copyObject.chapter}.{activities.versesRange(activities.copyObject.verses)}?mode=launcher&src=bolls&utm_source=bolls&utm_medium=share"
	...
	<self [y:{#dy}px @off:100% o@off:0 transition-duration:{transitionDuration}] ease
		...
		>
		<svg.chevron src=ICONS.CARET_DOWN @click=close>
		<header>
		...
		<menu>
						...
						if activities.show_sharing
							<.popup-menu [l:0 @lt-sm:0.5rem top:unset b:calc(100% + .25rem) y@off:2rem o@off:0 w:14rem] ease>
								...
								<button @click=shareViaFB>
									<svg src=ICONS.FACEBOOK_LOGO aria-hidden=yes>
									"Facebook"
								...
								<button @click=(do()
										...
										<svg src=ICONS.LINK aria-hidden=yes>
										t.copy_with_link
			...
			<li>
				<button @click=compare.load>
					<svg src=ICONS.ARROWS_SPLIT aria-hidden=yes>
					t.compare
			<li>
				<button @click=activities.openModal('notes')>
					<svg src=ICONS.NOTE_PENCIL aria-hidden=yes>
					t.note
			<li>
					...
					if activities.show_bookmarks
						<.popup-menu [r:0 @lt-sm:0.5rem top:unset b:calc(100% + 4px) y@off:2rem o@off:0 w:14rem] ease>
							...
							<button[d:hcc p:0.5rem 1rem mt:.25rem] @click=showAddNewCategory>
								<svg src=ICONS.PLUS aria-hidden=yes>
								t.new_tag
