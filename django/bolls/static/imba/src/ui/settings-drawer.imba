...
tag settings-drawer < aside
	...
	<self>
		...
		<button.settings-btn @click=activities.showSearch>
			<svg src=ICONS.MAGNIFYING_GLASS aria-hidden=true>
			t.bible_search
		<button.settings-btn @click=pageSearch.run>
			<svg src=ICONS.LIST_MAGNIFYING_GLASS aria-hidden=true>
			t.find_in_chapter
		<button.settings-btn @click=activities.showHistory>
			<svg src=ICONS.CHEESE aria-hidden=true>
			t.history
		<menu-popup bind=activities.show_themes>
			<button.settings-btn [pos:relative] @click=(do activities.show_themes = !activities.show_themes)>
				<svg src=ICONS.RAINBOW_CLOUD aria-hidden=true>
				t.theme
					...
					<.popup-menu [l:0 y@off:-1rem o@off:0] ease>
						...
						<button[d:hcs]
							@click=activities.openCustomTheme>
								t.createTheme
								<svg src=ICONS.PAINT_BRUSH [miw:1rem size:1rem mr:0] aria-hidden=true>
		...
		<.btnbox>
			<svg.cbtn role="button" @click=theme.changeLineHeight(no) viewBox="0 0 38 14" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" [p:1rem 0]>
				<title> t.decrease_line_height
				<rect x="0" y="0" width="28" height="1">
				<rect x="0" y="6" width="38" height="1">
				<rect x="0" y="12" width="18" height="1">
			<svg.cbtn role="button" @click=theme.changeLineHeight(yes) viewBox="0 0 38 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" [p:0.5rem 0]>
		...
		<menu-popup bind=activities.show_languages>
			<button.settings-btn [pos:relative] @click=(do activities.show_languages = !activities.show_languages)>
				<svg src=ICONS.TRANSLATE aria-hidden=true>
				currentLanguage
		...
		<button.settings-btn @click=reader.randomVerse>
			<svg src=ICONS.SHUFFLE aria-hidden=true>
			t.random

		if window.navigator.onLine
			if vault.available
				<button.settings-btn @click=activities.toggleDownloads>
					<svg src=ICONS.CLOUD_ARROW_DOWN aria-hidden=true>
			...
			<a.settings-btn href='/downloads/' target="_blank" @click=pwa.install>
				<img[size:2rem rd:23% mr:0.75rem] src='/static/bolls.png' aria-hidden=true>
				t.install_app
		<button.settings-btn @click=activities.showHelp>
			<svg src=ICONS.CARROT aria-hidden=true>
			t.help
		<button.settings-btn @click=activities.showSupport>
			<svg src=ICONS.HANDSHAKE aria-hidden=true>
			t.support

		unless !"state.pswv"
			<a.settings-btn route-to="/donate/">
				<svg src=ICONS.HAND_COINS aria-hidden=true>
				t.donate
		...
		<footer>
			...
			<p>
				"v3.3.7 | "
				<time dateTime='2026-5-10'> "10.5.2026"
