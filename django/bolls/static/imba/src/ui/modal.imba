...
tag modal < section
	...
	def downloadStatusIcon status\string
		switch status
			when 'processing'
				return <svg.spin src=ICONS.SPINNER_BALL aria-hidden=yes>
			when 'delete'
				return <svg src=ICONS.TRASH aria-hidden=yes>
			else
				return <svg src=ICONS.DOWNLOAD_SIMPLE aria-hidden=yes>
	...
	get backdropColor
		...
		const fallbackColor = window.getComputedStyle(document.documentElement).getPropertyValue('background-color')
				...
				switch activities.activeModal
					when 'help'
						<header>
							...
							<a target="_blank" href="mailto:bpavlisinec@gmail.com" title=t.help>
								<svg src=ICONS.PAPER_PLANE_TILT aria-hidden=yes>
						...
						<header [pos:relative]>
							...
							<menu-popup bind=activities.show_comparison_options>
								<button @click=(do activities.show_comparison_options = !activities.show_comparison_options) title=t.compare>
									<svg src=ICONS.LIST_PLUS aria-hidden=yes>
								if activities.show_comparison_options
									<.popup-menu [t:0 y@off:-1rem o@off:0 mah:72vh @lt-sm:96vh of:auto] ease>
										<header[d:flex bg:$bgc pos:sticky t:0 padding-inline-end:0.5rem]>
											...
											<button[p:0 w:1.5rem miw:1.5rem] title=t.close @click=(activities.show_comparison_options = no)>
												<svg src=ICONS.X [s:1.5rem c@hover:red4] aria-hidden=yes>
					...
					when 'downloads'
						<header>
							...
							<h2[transform@important:none pos:relative c@hover:$acc-hover fill:$c @hover:$acc-hover cursor:pointer d:flex w:100% jc:center ai:center us:none]
								...
								<svg[s:1.5rem] src=ICONS.CARET_DOWN aria-hidden=yes>

							if vault.deletingEverything
								<svg.spin src=ICONS.SPINNER_BALL>
									<title> t.loading
							else
					...
					when 'support'
						<header>
							...
							<h2> t.support
							<a target="_blank" href="mailto:bpavlisinec@gmail.com" title=t.help>
								<svg src=ICONS.PAPER_PLANE_TILT aria-hidden=true>
					...
					when "dictionary"
						<header[pos:relative]>
							...
							<button @click=openDictionaryDownloads title=t.download>
								<svg src=ICONS.DOWNLOAD_SIMPLE aria-hidden=yes>
						...
						<menu-popup bind=activities.show_dictionaries [pos:relative]>
							<[transform@important:none padding-block:.5rem c@hover:$acc-hover fill:$c @hover:$acc-hover cursor:pointer tt:uppercase fw:500 fs:0.9em d:hss]
								...
								<svg[s:1.5rem] src=ICONS.CARET_DOWN aria-hidden=yes [transform:rotateX(180deg)]=activities.show_dictionaries>
								...
								for definition, index in dictionary.definitions when index < 64
									...
									<li.definition id=definition.topic .expanded=expanded>
										<header @click=dictionary.expandDefinition(definition.topic)>
											...
											<svg[s:1.5rem] src=ICONS.CARET_DOWN aria-hidden=yes [transform:rotateX(180deg)]=expanded>
						...
						<header#search-header [pos:relative]>
							...
							if window.navigator.onLine
								<button.focusable title=t.match_case [o:0.5]=!search.match_case @click=(search.match_case = !search.match_case)>
									<svg src=ICONS.TEXT_AA aria-hidden=yes>
							...
							if search.results.length
								<menu-popup bind=activities.show_filters>
									<button
										...
										<svg src=ICONS.FUNNEL aria-hidden=yes>
